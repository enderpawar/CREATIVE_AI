...
- 대학생의 주간 학습 시간, 수면 시간, 이전 학기 학점을 기반으로 현재 학기의 예상 학점(평점)을 예측하는 선형 회귀(Regression) 문제 시나리오를 기준으로 파이썬코드를 출력하는 데모 영상을 보여드립니다. 사용된 csv는 현재 github 리포지토리 내 example 폴더에 있습니다
...
- 캔버스에 노드를 자동 배치하는 기능을 추가한 데모 영상입니다.
